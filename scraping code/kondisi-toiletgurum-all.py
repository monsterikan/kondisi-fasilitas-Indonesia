...
df = pd.DataFrame(columns = ['Kecamatan','Toilet Guru L Baik','Toilet Guru L Rusak Ringan','Toilet Guru L Rusak Sedang','Toilet Guru L Rusak Berat','Toilet Guru L Rusak Total','Toilet Guru L Total'])
...
for i in areaarea:
    ...
    for k in range(len(kecamatan)):
        df = df.append({'Kecamatan': kecamatan[k], 'Toilet Guru L Baik': baik[k], 'Toilet Guru L Rusak Ringan': rringan[k], 'Toilet Guru L Rusak Sedang': rsedang[k], 'Toilet Guru L Rusak Berat': rberat[k], 'Toilet Guru L Rusak Total': rtotal[k], 'Toilet Guru L Total': total[k]}, ignore_index = True)
    

df.to_csv('kondisi_toliet_guru_L_all.csv', index=False)

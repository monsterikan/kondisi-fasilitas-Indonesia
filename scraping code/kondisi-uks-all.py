...
df = pd.DataFrame(columns = ['Kecamatan','UKS Baik','UKS Rusak Ringan','UKS Rusak Sedang','UKS Rusak Berat','UKS Rusak Total','UKS Total'])
...
for i in areaarea:
    ...
    for k in range(len(kecamatan)):
        df = df.append({'Kecamatan': kecamatan[k], 'UKS Baik': baik[k], 'UKS Rusak Ringan': rringan[k], 'UKS Rusak Sedang': rsedang[k], 'UKS Rusak Berat': rberat[k], 'UKS Rusak Total': rtotal[k], 'UKS Total': total[k]}, ignore_index = True)
    
df.to_csv('kondisi_uks_all.csv', index=False)

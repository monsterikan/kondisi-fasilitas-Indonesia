...
df = pd.DataFrame(columns = ['Kecamatan','Toilet Murid L Baik','Toilet Murid L Rusak Ringan','Toilet Murid L Rusak Sedang','Toilet Murid L Rusak Berat','Toilet Murid L Rusak Total','Toilet Murid L Total'])
...
for i in areaarea:
    ...
    for k in range(len(kecamatan)):
        df = df.append({'Kecamatan': kecamatan[k], 'Toilet Murid L Baik': baik[k], 'Toilet Murid L Rusak Ringan': rringan[k], 'Toilet Murid L Rusak Sedang': rsedang[k], 'Toilet Murid L Rusak Berat': rberat[k], 'Toilet Murid L Rusak Total': rtotal[k], 'Toilet Murid L Total': total[k]}, ignore_index = True)

df.to_csv('kondisi_toliet_murid_L_all.csv', index=False)

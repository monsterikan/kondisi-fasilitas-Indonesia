...
df = pd.DataFrame(columns = ['Kecamatan','Perpus Baik','Perpus Rusak Ringan','Perpus Rusak Sedang','Perpus Rusak Berat','Perpus Rusak Total','Perpus Total'])

for i in areaarea:
    url = ('http://statistik.data.kemdikbud.go.id//index.php/statistik/table/sma/2020/' + str(i) + '/0/28')
    ...
    for k in range(len(kecamatan)):
        df = df.append({'Kecamatan': kecamatan[k], 'Perpus Baik': baik[k], 'Perpus Rusak Ringan': rringan[k], 'Perpus Rusak Sedang': rsedang[k], 'Perpus Rusak Berat': rberat[k], 'Perpus Rusak Total': rtotal[k], 'Perpus Total': total[k]}, ignore_index = True)
    

df.to_csv('kondisi_perpus_all.csv', index=False)

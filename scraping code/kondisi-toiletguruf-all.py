...
df = pd.DataFrame(columns = ['Kecamatan','Toilet Guru P Baik','Toilet Guru P Rusak Ringan','Toilet Guru P Rusak Sedang','Toilet Guru P Rusak Berat','Toilet Guru P Rusak Total','Toilet Guru P Total'])
...
for i in areaarea:
    ...
    for k in range(len(kecamatan)):
        df = df.append({'Kecamatan': kecamatan[k], 'Toilet Guru P Baik': baik[k], 'Toilet Guru P Rusak Ringan': rringan[k], 'Toilet Guru P Rusak Sedang': rsedang[k], 'Toilet Guru P Rusak Berat': rberat[k], 'Toilet Guru P Rusak Total': rtotal[k], 'Toilet Guru P Total': total[k]}, ignore_index = True)

df.to_csv('kondisi_toliet_guru_P_all.csv', index=False)
